...
class SettingsDialog(tk.Toplevel):
    ...
    def _build_survey_tab(self):
        ...
        self.survey_columns = ("name", "start", "end")
        self.survey_list = ttk.Treeview(frame, columns=self.survey_columns, show="headings", height=10)
    ...
    def _reload_surveys(self):
        ...
        autosize_columns(self.survey_list, self.survey_columns)
    ...
    def _build_common_tab(self):
        ...
        self.rate_columns = ("year", "daily", "meal")
        self.rate_list = ttk.Treeview(frame, columns=self.rate_columns, show="headings", height=10)
        for c, label in [("year", "연도"), ("daily", "일급(원)"), ("meal", "월 식대(원)")]:
    ...
    def _reload_rates(self):
        ...
        for year in self.config_obj.rate_years():
            try:
                daily_display = f"{self.config_obj.daily_wage_for(year):,}"
            except ValueError:
                daily_display = "재입력 필요"
            try:
                meal_display = f"{self.config_obj.meal_allowance_for(year):,}"
            except ValueError:
                meal_display = "재입력 필요"
            self.rate_list.insert("", "end", values=(year, daily_display, meal_display))
        autosize_columns(self.rate_list, self.rate_columns)

    def _on_rate_select(self, _evt=None):
        sel = self.rate_list.selection()
        if not sel:
            return
        year, daily, meal = self.rate_list.item(sel[0], "values")
        self.rate_year_var.set(year)
        self.rate_daily_var.set(str(daily).replace(",", ""))
        self.rate_meal_var.set(str(meal).replace(",", ""))

    def _add_or_update_rate(self):
        try:
            year = int(self.rate_year_var.get())
            daily = int(self.rate_daily_var.get().replace(",", ""))
            meal = int(self.rate_meal_var.get().replace(",", ""))
        except ValueError:
            messagebox.showerror("형식 오류", "연도/일급/식대는 모두 숫자로 입력하세요.")
            return
        self.config_obj.set_year_rates(year, daily, meal)
        self.config_obj.save()
        self._reload_rates()
