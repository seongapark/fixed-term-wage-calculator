...
| `배포안내_README.txt` | 제목, 받을 파일 세 개, 이번 버전의 변경 안내 |
...
cp ../../배포안내_README.txt ./README.txt            # 공유문서함 게시용 안내문
...
`README.txt`의 원본은 저장소 루트의 `배포안내_README.txt`다(dist는 gitignore라 버전관리가
안 된다). **버전을 올릴 때 이 파일도 같이 고친다** — 파일명 세 개, "처음 vX.Y로 계산하실 때만"
같은 전환 안내, 그 버전에서 계산 결과가 달라지는 항목(담당자가 금액 차이를 보고 놀라지
않도록), 그리고 맨 아래 문의처.
...
**exe + README + 변경이력 + 로직 안내문을 함께** 올리는 것으로 끝난다.
