...
| 코드 규모 | 파이썬 약 6,600 LOC / 자동화 테스트 148개(46개 파일) 전부 통과 / 커밋 112건 |
...
▶ **[docs/assets/demo.mp4](docs/assets/demo.mp4)** — 설정 등록부터 엑셀 산출까지 전체 흐름 (1분 2초, 자막 포함)

영상 목차: `1. 설정 관리(최초 1회)` → `2. 파일 첨부` → `3. 대상자 확인` → `4. 계산 실행` → `5. 계산 결과·산정근거 확인`
...
| 특별휴가 유급/무급 | 종별만 보고 담당자가 임의 판단 | 파일 불러온 직후 **건별로 유급/무급을 물어봄** | 판단 근거가 화면에 남음 |
...
### 4-3. 근무상황 종별 자동 처리 (17종 매핑 내장)
| 종별 | 급여 | 주휴·연가 발생 |
|---|---|---|
| 결근 | 무급(일급·식대 미지급) | 깨짐 |
| 공가(소환·예방접종·자녀돌봄 등) | 유급 | 영향 없음 |
| 연가 / 반일연가 | 유급 | 영향 없음 (잔여에서 480분/240분 차감) |
| 병가(종일, 진단서 무관) | 급여일수 무관 | 깨짐(실근무 0분) |
| 조퇴·지각·외출(시간 기재) | 비운 시간만 시급 공제 | **깨지 않음** (v5.3) |
| 유급 특별휴가(경조사·포상 등) | 유급 | 영향 없음 |
| 무급 특별휴가(가족돌봄·생리휴가 등) | 무급 | 깨지 않음 |
| 기타 | 영향 없음 | 영향 없음 |

- 모든 시간 계산은 **1분 단위**. 반올림 없이 처리.
- **특별휴가 유급/무급 구분 화면**: 종별에 "특별휴가"만 찍혀 구분이 없는 건은 파일을 불러온 직후 건별로 물어보고, 선택값이 그대로 계산에 반영됨.
...
- **자동화 테스트 148개(46개 파일, 전부 통과)** — 주휴 판정, 연가 발생·상계 시뮬레이션, 소급 재계산(당월/퇴사자/연가상계), 파서(동명이인·날짜형 생년월일·전월 임금내역), 엑셀 산출물, 웹앱 상태 전이까지 회귀 고정.
...
- **로직 안내문 동봉** — [`임금계산_로직_안내(v5.3).txt`](임금계산_로직_안내(v5.3).txt), [`변경이력.txt`](변경이력.txt)를 exe와 함께 배포해 담당자가 산정 기준을 문서로 확인.
...
| v5.4 | 예정 | 조퇴/외출/지각의 연가 상계, 연가 부족 경고 팝업, 휴무일 겹친 공휴일 표기 정정 |
