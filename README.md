...
| 코드 규모 | 파이썬 약 6,600 LOC / 자동화 테스트 221개(55개 파일) 전부 통과 / 커밋 129건 |
...
- **로직 안내문 동봉** — [`임금계산_로직_안내(v5.5).txt`](<임금계산_로직_안내(v5.5).txt>), [`변경이력.txt`](변경이력.txt)를 exe와 함께 배포해 담당자가 산정 기준을 문서로 확인.
