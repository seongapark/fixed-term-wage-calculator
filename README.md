...
| 코드 규모 | 파이썬 약 6,600 LOC / 자동화 테스트 221개(55개 파일) 전부 통과 / 커밋 112건 |
...
기간제 조사관 근무상황에 실제로 찍히는 종별 9개만 인식합니다. 표기가 이 9개와 정확히 맞지 않으면
임의로 추정하지 않고 확인 화면으로 보냅니다.

| 종별 | 일급 | 식대 | 연가 잔량 | 주휴·연가 발생 |
|---|---|---|---|---|
| 연가 | 미공제 | 지급 | 480분 차감 | 영향 없음 |
| 반일연가 | 미공제 | 지급 | 240분 차감 | 영향 없음 |
| 공가 | 미공제 | 지급 | — | 영향 없음 |
...
- **자동화 테스트 221개(55개 파일, 전부 통과)** — 주휴 판정, 연가 발생·상계 시뮬레이션, 소급 재계산(당월/퇴사자/연가상계), 파서(동명이인·날짜형 생년월일·전월 임금내역), 엑셀 산출물, 웹앱 상태 전이까지 회귀 고정.
...
- **로직 안내문 동봉** — [`임금계산_로직_안내(v5.4).txt`](<임금계산_로직_안내(v5.4).txt>), [`변경이력.txt`](변경이력.txt)를 exe와 함께 배포해 담당자가 산정 기준을 문서로 확인.
...
| v5.4 | 예정 | 조퇴/외출/지각의 연가 상계와 부족 경고, 종별 9종 확정(그 밖은 사람이 판단), `기타` 분리와 종일 `기타` 처리, 확인 화면 2항목화, 전월 결과파일로 근무상황·판정 이어받기 |
